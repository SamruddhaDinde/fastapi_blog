...
async def create_post(post: PostCreate, 
  ...
  #   )
  # we do not need above code cause Current User1 will check if the user is right, other wise it unauthorized status
...
#      )
# we dont need above code because, Current User check athorization
...
async def update_post_partial(post_id: int, post_data: PostUpdate, current_user: CurrentUser, db: Annotated[AsyncSession, Depends(get_db)]):
 ...
 #with eclude unset we will only modify what the user wangts
